$(document).on('click', '.stock_setup_auto_fill', function () {
    const $button = $(this);
    ...
    const $container = $('.stock_wrapper').find('.outline-wrapper');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            const stockType = data.stock_type || 'unlimited';
            const productStock = data.product_stock || '';

            // Update form fields
            $('#stock_type').val(stockType).trigger('change');
            $('#product_stock').val(productStock);

            if ($button.data('next-action')?.toString() === 'addon-setup') {
        ...
        complete: function () {
            ...
            if ($button.data('next-action')?.toString() === 'addon-setup') {
                setTimeout(function () {
                    const target = document.querySelector('.addon_setup_auto_fill');
                    if (target) {
                        target.setAttribute('data-next-action', 'tags-setup');
                        target.click();
                    }
                }, 2000);
            }
