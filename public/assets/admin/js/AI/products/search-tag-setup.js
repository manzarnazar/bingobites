$(document).on('click', '.search_tag_setup_auto_fill', function () {
    const $button = $(this);
    ...
    const $container = $('.tags_wrapper').find('.outline-wrapper');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if ($button.data('next-action')?.toString() === 'cuisine-setup') {
        ...
        complete: function () {
            ...
            if ($button.data('next-action')?.toString() === 'cuisine-setup') {
                setTimeout(function () {
                    const target = document.querySelector('.cuisine_setup_auto_fill');
                    if (target) {
                        target.setAttribute('data-next-action', 'variation-setup');
                        target.click();
                    }
                }, 2000);
            }
