        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex flex-wrap justify-content-between align-items-center">
                    ...
                    <a class="btn-sm btn btn-outline-primary p-2 cursor-pointer cmn_focus" data-toggle="modal" data-target="#instructionsModal" tabindex="1">
                ...
                <div class="mt-4">
                    ...
                    <form
                        ...
                        <div class="d-flex flex-wrap mb-4 pl-2 cmn_focus py-1 px-2 rounded">
                            <label class="form-check mb-0 form--check mr-2 mr-md-4">
                                <input class="form-check-input" type="radio" name="status" id="captcha_active" value="1" tabindex="2" {{isset($config) && $config['status']==1?'checked':''}}>
                                <span class="mb-0">{{translate('active')}}</span>
                            </label>
                            <label class="form-check mb-0 form--check">
                                <input class="form-check-input" type="radio" name="status" id="captcha_inactive" value="0" tabindex="3" {{isset($config) && $config['status']==0?'checked':''}}>
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="site_key" tabindex="4" value="{{env('APP_MODE')!='demo'?$config['site_key']??"":''}}">
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label class="text-capitalize">{{translate('Secret Key')}}</label><br>
                                    <input type="text" class="form-control" name="secret_key" tabindex="5" value="{{env('APP_MODE')!='demo'?$config['secret_key']??"":''}}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="btn--container">
                            <button type="{{env('APP_MODE')!='demo'?'submit':'button'}}"
                                    class="btn btn-primary call-demo" tabindex="6">{{translate('save')}}</button>
                        </div>
                    </form>
                </div>
            </div>
