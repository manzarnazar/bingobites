            ...
            <div class="col-lg-4">
                                        ...
                                        <div class="media flex-wrap gap-3">
                                            <a>
                                                <img class="avatar avatar-lg rounded-circle"
                                                     src="{{$order->delivery_man?->imageFullPath }}" alt="Image">
                                            </a>
                                            <div class="media-body d-flex flex-column gap-1">
                                                <a target="" href="#"
                                                   class="text-dark"><span>{{$order->delivery_man['f_name'].' '.$order->delivery_man['l_name'] ?? ''}}</span></a>
                                                <span
                                                    class="text-dark"> <span>{{$order->delivery_man['orders_count']}}</span> {{translate('Orders')}}</span>
                            ...
                            @if($order->customer)
                                <div class="media flex-wrap gap-3">
                                    ...
                                    <div class="media-body d-flex flex-column gap-1">
                                        <a target="_blank"
                                           href="{{route('admin.customer.view',[$order->customer['id']])}}"
                                           class="text-dark"><strong>{{$order->customer['f_name'].' '.$order->customer['l_name']}}</strong></a>
                                        <span
                                            class="text-dark">{{$order->customer['orders_count']}} {{translate('Orders')}}</span>
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        <div class="media flex-wrap gap-3">
                            <div class="">
                                <img class="avatar avatar-lg rounded-circle" src="{{ $order->branch?->imageFullPath}}"
                                     alt="Image">
                            </div>
                            <div class="media-body d-flex flex-column gap-1">
...
@push('script_2')
    ...
    <script>
        ...
        function openProductModal(productId, branchId, orderId) {
            $.ajax({
                url: '{{ route('admin.orders.product.variation') }}',
                type: 'GET',
                data: {
        ...
        $(document).on('click', '.edit-product-quantity-btn', function () {
            ...
            $.ajax({
                url: '{{ route('admin.orders.update-product-quantity') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    order_id: orderId,
        ...
        $(document).on('click', '.product-variation-add-to-cart-button', function () {
            ...
            $.ajax({
                url: '{{ route("admin.orders.addProductToSession") }}',
                method: 'POST',
                data: formData,
                success: function (response) {
                    if (response.success) {
        ...
        $(document).on('click', '.delete_product', function () {
            ...
            $.ajax({
                url: '{{ route('admin.orders.delete-product-from-session') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
