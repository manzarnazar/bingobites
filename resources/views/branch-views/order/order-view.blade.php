...
@section('content')
                            ...
                            @foreach($order->details as $detail)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        {{\App\CentralLogics\Helpers::set_symbol($product_tax+ $detail['add_on_tax_amount'])}}
                                    </td>
    ...
    <div class="offcanvas z--999" id="editOrderOffcanvas" style="--width: 850px">
        ...
        <div class="offcanvas-body px-4 pb-0 pt-4">
            ...
            @include('branch-views.order.partials.order-products-table')
        </div>
...
@push('script_2')
    ...
    <script>
        ...
        $(document).on('click', '#edit-order-button', function () {
            $.ajax({
                url: '{{ route('branch.orders.clear-order-edit-session') }}',
                type: 'GET',
        ...
        function loadEditOrderProducts(orderId) {
            $.ajax({
                url: '/branch/orders/edit/' + orderId,
                type: 'GET',
                success: function (response) {
                    if (response.success) {
        ...
        $(document).on('input change', '.edit-order-product-search-input', function () {
            ...
            $.ajax({
                url: '{{ route("branch.orders.search-product") }}',
                type: 'GET',
                data: {
                    search: search,
                    order_id: orderId
        ...
        function openProductModal(productId, branchId, orderId) {
            $.ajax({
                url: '{{ route('branch.orders.product.variation') }}',
                type: 'GET',
                data: {
        ...
        $(document).on('click', '.edit-product-quantity-btn', function () {
            ...
            $.ajax({
                url: '{{ route('branch.orders.update-product-quantity') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    order_id: orderId,
        ...
        $(document).on('click', '.product-variation-add-to-cart-button', function () {
            ...
            $.ajax({
                url: '{{ route("branch.orders.addProductToSession") }}',
                method: 'POST',
                data: formData,
                success: function (response) {
                    if (response.success) {
        ...
        $(document).on('click', '.delete_product', function () {
            ...
            $.ajax({
                url: '{{ route('branch.orders.delete-product-from-session') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
        ...
        $(document).on('click', '.update-edit-order', function () {
            ...
            $.ajax({
                url: '{{ route('branch.orders.update-edit-order') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    order_id: orderId,
