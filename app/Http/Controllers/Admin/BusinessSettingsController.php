...
class BusinessSettingsController extends Controller
{
    ...
    public function updateFcmMessages(Request $request): RedirectResponse
    {
        ...
        $this->InsertOrUpdateBusinessData(['key' => 'order_edit_message'], [
            'value' => json_encode([
                'status' => $request['edit_status'] == 1 ? 1 : 0,
                'message' => $request['edit_message'],
            ]),
        ]);
        $editOrder = $this->business_setting->where('key', 'order_edit_message')->first();

        foreach ($request->lang as $index => $key) {
            if ($key === 'default') {
                continue;
            }
            $message = $request->order_edit_message[$index - 1] ?? null;
            if ($message !== null) {
                Translation::updateOrInsert(
                    [
                        'translationable_type' => 'App\Model\BusinessSetting',
                        'translationable_id' => $editOrder->id,
                        'locale' => $key,
    ...
    public function customerSettingsUpdate(Request $request): RedirectResponse
    {
        ...
        if ($request->customer_referred_discount_type == 'percent' && (($request->customer_referred_discount_amount ?? 0) > 100)) {
            Toastr::error(translate('The referral discount percentage cannot be more than 100%.'));
            return back();
        }
